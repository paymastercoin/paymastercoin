Paymastercoin Core version *0.15.1* is now available from:

  <https://download.pmcoin.biz/paymastercoin-0.15.1/>
...
A number of changes to the way Paymastercoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mPMC per kB (0.00001 PMC). This is to prepare for dropping the minimum transaction fee to 0.1 mPMC per kB (0.0001 PMC) in the next release.
