...
Below are some notes on how to build Paymastercoin Core for Windows.

The options known to work for building Paymastercoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Paymastercoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Paymastercoin Core applications.
...
Note that for WSL the Paymastercoin Core source path MUST be somewhere in the default mount file system, for
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/paymastercoin/paymastercoin.git
    sudo chmod -R a+rw paymastercoin
...
causes two of the paymastercoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the paymastercoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the paymastercoin source code).
